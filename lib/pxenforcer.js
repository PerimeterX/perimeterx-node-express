...
const MODULE_VERSION = 'NodeJS Module v7.0.3';
                ...
                enforcer.enforce(req, res, (err, response) => {
                    if (!err && response) {
                        ...
                        res.on('finish', () => {
                            ...
                            const { AUTOMATIC_ADDITIONAL_ACTIVITY } = enforcer.config.conf;
                            const { pxCtx } = req.locals;
                            if (AUTOMATIC_ADDITIONAL_ACTIVITY && pxCtx.additionalFields.loginCredentials) {
