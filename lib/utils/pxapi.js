 ...
 * @param {string} vid - user's visitor id.
 ...
 * @param {string} http_version - incoming request http version
 * @param {Function} callback - callback function.
 */
function callServer(ip, vid, headers, uri, cause, http_version, callback) {
    const data = {
        request: {
            ip: ip,
            headers: headers,
...
function evalByServerCall(pxCtx, cause, callback) {
    ...
    callServer(ip, vid, headers, uri, cause, http_version, (err, res) => {
        if (err) {
            if (err === 'timeout') {
