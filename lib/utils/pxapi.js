...
function callServer(ip, headers, uri, cause, callback) {
    const riskCall = client.post(pxconfig.SERVER_HOST + pxconfig.SERVER_TO_SERVER_API_URI, {
        data: {
            ...
            call_reason: causeToString(cause)
        },
        headers: {
    ...
    riskCall.on('timeout', function () {
        return callback(`perimeterx server query timeout`);
    });
}
 ...
 * @param {Function} callback - callback function.
 */
function evalByServerCall(ip, headers, uri, cause, callback) {
    if (!ip || !headers || !uri) {
        pxlogger.error('perimeterx score evaluation failed. bad parameters.');
        return callback(pxconfig.API_EVAL.UNEXPECTED_RESULT);
    }
    callServer(ip, headers, uri, cause, (err, res) => {
        if (err) {
            pxlogger.error(err.message);
            return callback(pxconfig.API_EVAL.UNEXPECTED_RESULT);
        }
        ...
        if (action === 1) {
            return callback(pxconfig.API_EVAL.S2S_GOOD_SCORE);
        }

        /* score crossed threshold - block traffic */
        if (action === 0) {
            return callback(pxconfig.API_EVAL.S2S_BAD_SCORE);
        }
...
function causeToString(cause) {
    switch (cause) {
        case pxconfig.COOKIE_EVAL.COOKIE_INVALID:
            return 'cookie_invalid';
        case pxconfig.COOKIE_EVAL.COOKIE_EXPIRED:
            return 'cookie_expired';
        case pxconfig.COOKIE_EVAL.NO_COOKIE:
            return 'no_cookie';
        case pxconfig.COOKIE_EVAL.UNEXPECTED_RESULT:
        default:
            return 'none';
