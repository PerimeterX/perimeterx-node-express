...
function callServer(pxCtx, callback) {
    ...
    if (pxCtx.s2s_call_reason == 'cookie_decryption_failed') {
        data.additional.px_orig_cookie = pxCtx.px_orig_cookie; //No need strigify, already a string
    }

    if (pxCtx.s2s_call_reason == 'cookie_expired' || pxCtx.s2s_call_reason == 'cookie_validation_failed') {
...
function isBadRiskScore(res, pxCtx) {
    ...
    if (score >= pxconfig.BLOCKING_SCORE) {
        ...
        pxCtx.block_action = res.action;
        pxCtx.uuid = res.uuid;
        if (res.action === 'j' && res.action_data && res.action_data.body) {
            pxCtx.block_action_data = res.action_data.body;
        }
        return 0;
    } else {
        return 1;
