...
function evalCookie(pxCtx) {
    try {
        ...
        const cookie = Cookie.pxCookieFactory(pxCtx, pxconfig);
        ...
        pxCtx.decrypted_px_cookie = cookie.decodedCokie;
        pxCtx.block_score = cookie.score;
        pxCtx.px_vid = cookie.vid;
        pxCtx.block_uuid = cookie.uuid;
        ...
        if (!cookie.isSecure()) {
            pxlogger.debug('cookie invalid ' + JSON.stringify(cookie.decodedCokie));
