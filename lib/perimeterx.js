        ...
        if (req.cookies && req.cookies['_pxCaptcha']) {
            ...
            pxCaptcha.verifyCaptcha(pxCtx, pxcptch, (err, result) => {
                if (err) {
                    if (err === "timeout") {
                        ...
                        return pxBlock(pxCtx, pxconfig.SCORE_EVALUATE_ACTION.CAPTCHA_BLOCK_TRAFFIC);
                    }
                }
            ...
            pxutil.verifyUserScore(pxCtx, (action) => {
                ...
                if (pxconfig.CUSTOM_REQUEST_HANDLER){
                  pxconfig.CUSTOM_REQUEST_HANDLER(pxCtx, req, res);
                  return next();
                }

                if (pxCtx.score < pxconfig.BLOCKING_SCORE || pxconfig.MODULE_MODE === pxconfig ){
                    pxPass(pxCtx);
                    return next();
                } else {
                    pxBlock(pxCtx, action);
