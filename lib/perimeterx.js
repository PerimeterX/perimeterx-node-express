...
function pxMiddleware(req, res, next) {
    ...
    if (req.method !== 'GET' || pxutil.checkForStatic(req, pxconfig.STATIC_FILES_EXT)) {
        return next();
    }
    try {
        const pxCtx = {};
        ...
        if (req.cookies && req.cookies['_pxCaptcha']) {
            pxCaptcha.verifyCaptcha(pxCtx, req.cookies['_pxCaptcha'], (err, result) => {
                return pxBlock(pxCtx, '', pxconfig.BLOCK_HANDLER, req, res, next);
            });
        } else {
