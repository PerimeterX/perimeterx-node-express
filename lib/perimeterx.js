        ...
        if (req.cookies && req.cookies['_pxCaptcha']) {
            ...
            pxutil.verifyUserScore(pxCtx, (action) => {
                ...
                if (action === pxconfig.SCORE_EVALUATE_ACTION.COOKIE_PASS_TRAFFIC) {
                    ...
                    pxClient.sendToPerimeterX('page_requested', { 'px_cookie':pxCtx.decrypted_px_cookie }, pxCtx);
                    return next();
                }
...
function pxBlock(pxCtx, action, customHandler, req, res, next) {
    ...
    generateTemplate(pxCtx, pxconfig, (htmlTemplate) => {
 ...
 * @param {Callback} cb - hold the generated html value.
 */
function generateTemplate(pxContext, pxConfig, cb){
  const template = pxConfig.CAPTCHA_ENABLED ? 'captcha' : 'block'
  const templatesPath = `${__dirname}/templates`;
  let htmlTemplate = '';

  // Mustache preparations
  mu.root = templatesPath;
  let props = getProps(pxContext, pxConfig);
  let compile = mu.compileAndRender(`${template}.mustache`, props);
...
function getProps(pxContext, pxConfig){
  return {
    refId: pxContext.block_uuid,
    appId: pxConfig.PX_APP_ID,
    vid: pxContext.px_vid,
    uuid: pxContext.block_uuid,
