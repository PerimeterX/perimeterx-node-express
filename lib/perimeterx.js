...
function pxBlock(pxCtx, action, customHandler, req, res, next) {
    ...
    switch (action) {
        ...
        {
            reason = pxCtx.block_action === 'j' ? 'challenge' : 's2s_high_score';
            break;
        }
...
function generateTemplate(pxContext, pxConfig, cb){
    let template;
    if (pxContext.block_action === 'j') {
        template = 'challenge';
    } else if (pxContext === 'b' || !pxConfig.CAPTCHA_ENABLED) {
        template = 'block';
    } else {
        template = 'captcha';
    }
    ...
    if (template === 'challenge') {
        return cb(pxContext.block_action_data);
    }
